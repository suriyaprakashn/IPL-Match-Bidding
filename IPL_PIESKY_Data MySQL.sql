...
select bd.bidder_id 'Bidder ID', bd.bidder_name 'Bidder Name', 
(select count(*) from ipl_bidding_details bgd
where bgd.bid_status = 'won' and bd.bidder_id = bgd.bidder_id) / 
(select no_of_bids from ipl_bidder_points bp
where bp.bidder_id = bd.bidder_id)*100 as 'Percentage of Wins (%)'
from ipl_bidder_details bd order by 3 desc;

-- 2. Display the number of matches conducted at each stadium with stadium name, city from the database.
select * from ipl_match_schedule;
select * from ipl_match;
select * from ipl_stadium;
select msc.stadium_id,mst.stadium_name,mst.city,count(msc.match_id) total_matches
from ipl_match_schedule msc join ipl_stadium mst
on msc.stadium_id =  mst.stadium_id
group by msc.stadium_id
order by 1;
...
from ipl_match m join ipl_match_schedule msc 
on m.match_id=msc.match_id 
where m.toss_winner=m.match_winner and msc.stadium_id= s.stadium_id)/
(select count(msc.schedule_id) from ipl_match_schedule msc where msc.stadium_id= s.stadium_id)*100 '% of winner who won the toss'
from ipl_stadium s;
...
select bd.bid_team,t.team_name,count(bd.bid_team) total_bid
from ipl_bidding_details bd join ipl_team t on bd.bid_team = t.team_id group by bd.bid_team order by 1;

-- 5.	Show the team id who won the match as per the win details.
select ts.team_id,t.team_name,ts.matches_won from ipl_team_standings ts inner join
 ipl_team t on ts.team_id = t.team_id
 group by ts.team_id order by 3 desc limit 1;
 
-- 6. Display total matches played, total matches won and total matches lost by team along with its team name.
select ts.team_id,t.team_name,ts.matches_played,ts.matches_won,ts.matches_lost,ts.matches_tied from ipl_team_standings ts inner join
 ipl_team t on ts.team_id = t.team_id
 group by ts.team_id order by 1  ;
 select * from ipl_team_players;
 
 -- 7.	Display the bowlers for Mumbai Indians team.
 select p.player_id,p.player_name ,t.team_name,tp.player_role
 from ipl_player p join ipl_team_players tp on
 p.player_id = tp.player_id join
 ...
 -- 8.   How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order
  select t.team_name,count(p.player_name) total_all_rounder
 from ipl_player p join ipl_team_players tp on
 p.player_id = tp.player_id join
 ipl_team t on tp.team_id = t.team_id
 where tp.player_role='all-rounder' group by t.team_name
 having count(p.player_name)>4 order by 2 desc;